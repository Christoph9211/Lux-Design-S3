...
def path_to_actions(path, space):
    actions = []
    if not path:
        return actions
...
def handle_collisions_and_sap(fleet, opp_fleet, space):
    for ship in fleet:
        if ship.task == "move":
            for opp_ship in opp_fleet:
                if opp_ship.coordinates == ship.target.coordinates:
                    # Collision detected, handle it
                    if ship.energy > opp_ship.energy:
